...
public:
    ...
    KeyState GetKeyState(char key) {
        int keyIndex = kpd.findInList(key);
        if(keyIndex >= 0) {
            Key key = kpd.key[keyIndex];
            return key.kstate;
        }
...
public:
    ...
    KeyState GetKeyState(int key) {
        ...
        KeyState state = IDLE;
        if(key >= 0 && key < 13) {
            if((currentTouched & (1<<key)) && !(lastTouched & (1<<key))) state = PRESSED;
            if((currentTouched & (1<<key)) && (lastTouched & (1<<key))) state = HOLD;
            if(!(currentTouched & (1<<key)) && (lastTouched & (1<<key))) state = RELEASED;
        }
        return state;
    }
};
...
namespace XRMain
{
    ...
    void update() {
        ...
        // update all the control state and clear the screen
        hw.Update();

        // display some button states on the screen using nice helpers
        sprintf(CHARBUF, "PERFORM: %s", hw.buttons.GetKeyState(Buttons::ButtonNames::BTN_PERFORM_SONG) == KeyState::HOLD ? "held" : "");
        hw.display.drawStr(35, 0, CHARBUF);
        sprintf(CHARBUF, "PATTERN: %s", hw.buttons.GetKeyState(Buttons::ButtonNames::BTN_PATTERN_BANK) == KeyState::HOLD ? "held" : "");
        hw.display.drawStr(35, 10, CHARBUF);
        sprintf(CHARBUF, "TRACK: %s", hw.buttons.GetKeyState(Buttons::ButtonNames::BTN_TRACK_LAYER) == KeyState::HOLD ? "held" : "");
        hw.display.drawStr(35, 20, CHARBUF);

        // display all the encoders with their delta values and names
        for(int i=0; i<Encoders::ENC_NAME_LAST; i++) {
            sprintf(CHARBUF, "%s: %d", hw.encoders._ENC_NAME_STRINGS[i], hw.encoders.deltas[i].GetDelta());
            hw.display.drawStr(0, 10*i, CHARBUF);
        }

        // update the screen
        hw.display.sendBuffer();

        // update the LEDs to show various control states
        for(int i=0; i<22; i++) {
            bool ledState = hw.buttons.GetKeyState(i) == KeyState::HOLD;
            if(i<13) ledState = ledState || hw.keyboard.GetKeyState(i) == Keyboard::KeyState::HOLD;
            hw.leds.setPWM(i, ledState ? 4095 : 0);
        }

        // delay processing so that we can see the encoder deltas work
        delay(100);
    }
}
