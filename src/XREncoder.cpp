...
namespace XREncoder
{
    ...
    void handleEncoderTraversePages(int diff)
    {
        ...
        if (newPage != tPage) {
            ...
            int maxPages = currUXMode == XRUX::PATTERN_WRITE ? 4 : XRSound::getPageCountForCurrentTrack();
    ...
    void handleEncoderPatternModA(int diff)
    {
        ...
        case 2:
            {
                float currCompAtt = compAtt;
                float newCompAtt = currCompAtt + diff * 0.01;

                newCompAtt = constrain(newCompAtt, 0, 4.0);

                if (newCompAtt != currCompAtt)
                {
                    compAtt = newCompAtt;

                    AudioNoInterrupts();

                    // compressorLeft1.compression(compThr, compAtt, compRel, compRat, compKnee);
                    // compressorRight1.compression(compThr, compAtt, compRel, compRat, compKnee);
        ...
        case 3:
            {
                float currCompKnee = compKnee;
                float newCompKnee = currCompKnee + diff;

                newCompKnee = constrain(newCompKnee, 0, 32);

                if (newCompKnee != currCompKnee)
                {
                    compKnee = newCompKnee;
    ...
    void handleEncoderPatternModB(int diff)
    {
        ...
        case 2:
            {
                float currCompRel = compRel;
                float newCompRel = currCompRel + (diff * 0.01);

                newCompRel = constrain(newCompRel, 0, 4.0);

                if (newCompRel != currCompRel)
                {
                    compRel = newCompRel;
        ...
        case 3:
            {
                float currCompMkup = compMkup;
                float newCompMkup = currCompMkup + diff;

                newCompMkup = constrain(newCompMkup, -12, 24);

                if (newCompMkup != currCompMkup)
                {
                    compMkup = newCompMkup;

                    AudioNoInterrupts();

                    // compressorLeft1.makeupGain(compMkup);
                    // compressorRight1.makeupGain(compMkup);
    ...
    void handleEncoderPatternModC(int diff)
    {
        ...
        case 2:
            {
                float currCompRat = compRat;
                float newCompRat = currCompRat + diff;

                newCompRat = constrain(newCompRat, 1, 60);

                if (newCompRat != currCompRat)
                {
                    compRat = newCompRat;
    ...
    void handleEncoderPatternModD(int diff)
    {
        ...
        case 2:
            {
                float currCompThr = compThr;
                float newCompThr = currCompThr + diff;

                newCompThr = constrain(newCompThr, -40, 0);

                if (newCompThr != currCompThr)
                {
                    compThr = newCompThr;
